...
public class CrystalComputerItem extends HolderItem
{
	...
	public @NotNull InteractionResult useOn(UseOnContext context)
	{
		...
		if(!level.isClientSide())
			PacketHandlerInit.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) player), new ClientboundCrystalComputerOpenSaveScreenPacket(context.getHand() == InteractionHand.MAIN_HAND, context.getClickedPos()));
	...
	public @NotNull InteractionResultHolder<ItemStack> use(Level level, @NotNull Player player, @NotNull InteractionHand usedHand)
	{
		...
		PacketHandlerInit.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer) player), new ClientboundCrystalComputerOpenMainScreenPacket(usedHand == InteractionHand.MAIN_HAND));
