...
public class SGJourneyJigsawPlacement extends JigsawPlacement
{
															   ...
															   Optional<Heightmap.Types> projectStartToHeightmap,int maxDistanceFromCenter, Rotation rotation, PoolAliasLookup aliasLookup, DimensionPadding dimensionPadding, LiquidSettings liquidSettings)
	{
		...
		StructureTemplatePool structuretemplatepool = (StructureTemplatePool)startPool.unwrapKey().flatMap((p_314915_) -> registry.getOptional(aliasLookup.lookup(p_314915_))).orElse((StructureTemplatePool)startPool.value());
		...
		if (structurepoolelement == EmptyPoolElement.INSTANCE) {
			...
			if (startJigsawName.isPresent()) {
				...
				if (optional.isEmpty()) {
					StargateJourney.LOGGER.error("No starting jigsaw {} found in start pool {}", resourcelocation, startPool.unwrapKey().map((p_248484_) -> p_248484_.location().toString()).orElse("<unregistered>"));
					return Optional.empty();
				}
			...
			if (projectStartToHeightmap.isPresent()) {
				k = pos.getY() + chunkgenerator.getFirstFreeHeight(i, j, (Heightmap.Types)projectStartToHeightmap.get(), levelheightaccessor, context.randomState());
			} else {
				k = blockpos1.getY();
			}
